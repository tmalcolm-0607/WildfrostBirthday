...
public static class StatusEffect_WhenEnemyIsKilledApplyHealthToAttacker
{
    ...
    public static void Register(WildfrostBirthday.WildFamilyMod mod)
    {
        var builder = new StatusEffectDataBuilder(mod)
            .Create<StatusEffectApplyXWhenUnitIsKilled>("When Enemy Is Killed Apply Shell To Attacker")
            ...
            .WithTextInsert("<{a}><keyword=shell>")
            .WithStackable(true)
            ...
            {
                data.desc = "When an enemy is killed, apply <{0}><keyword=shell> to the attacker";
                data.effectToApply = mod.TryGet<StatusEffectShell>("Shell");
